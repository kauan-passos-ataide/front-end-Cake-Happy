    ...
    <div className=" w-full h-full flex flex-col items-center justify-center">
      <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {productsStore.products.map((item) => (
          <div
            key={item.id}
            className="bg-gray-100/70 rounded-lg px-3 py-5 flex flex-col items-center justify-center gap-2 max-w-[250px] shadow-black/30 shadow-lg"
          >
            <Image
              src={item.imageUrl}
              alt={item.name}
            ...
            <div className="w-full h-full flex flex-col justify-between">
              <h2 className="text-lg wrap-break-word">{item.name}</h2>
